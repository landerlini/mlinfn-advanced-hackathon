...
def test_geometry(Geometry, verbose=True):
    ...
    try:
        ...
        N1 = 100
        points = geo.sample_interior(key, N1)
        
        assert points.shape == (N1, 2), f"Expected shape ({N1}, 2), got {points.shape}"
        
        # Check x is in (0, L)
        x_coords = points[:, 0]
        assert jnp.all(x_coords >= 0) and jnp.all(x_coords <= L), \
            "x coordinates should be in [0, L]"
        
        # Check t is in (t_start, t_end)
        t_coords = points[:, 1]
        assert jnp.all(t_coords >= t_start) and jnp.all(t_coords <= t_end), \
            "t coordinates should be in [t_start, t_end]"
        
        # Check randomness: different calls should give different points
        points2 = geo.sample_interior(random.PRNGKey(43), N1)
        assert not jnp.allclose(points, points2), \
    ...
    try:
        key = random.PRNGKey(42)
        N2 = 100
        points = geo.sample_boundary(key, N2)
        
        assert points.shape == (N2, 2), f"Expected shape ({N2}, 2), got {points.shape}"
        
        # Check x is at boundaries (0 or L)
        x_coords = points[:, 0]
        at_boundaries = jnp.logical_or(
            jnp.isclose(x_coords, 0.0, atol=1e-6),
            jnp.isclose(x_coords, L, atol=1e-6)
        )
        assert jnp.all(at_boundaries), \
    ...
    try:
        key = random.PRNGKey(42)
        N3 = 100
        points = geo.sample_initial(key, N3)
        ...
        # Check t is 0
        t_coords = points[:, 1]
        assert jnp.allclose(t_coords, 0.0, atol=1e-6), \
    ...
    if hasattr(geo, 'sample_at_time'):
        try:
            ...
            N4 = 50
            t_test = 0.5
            points = geo.sample_at_time(key, N4, t_test)
            
            assert points.shape == (N4, 2), f"Expected shape ({N4}, 2), got {points.shape}"
            
            # Check t is t_test
            t_coords = points[:, 1]
            assert jnp.allclose(t_coords, t_test, atol=1e-6), \
...
def test_pinn(Pinn, verbose=True):
    ...
    try:
        ...
        assert hasattr(pinn, 'params'), "PINN must have attribute 'params'"
        assert len(pinn.params) == len(layers) - 1, \
            f"Expected {len(layers)-1} layer parameters, got {len(pinn.params)}"
    ...
    try:
        ...
        x_test = jnp.array([[1.0, 0.5], [2.0, 0.3], [3.0, 0.7]])
        output = pinn(x_test)
    ...
    try:
        x1 = jnp.array([[1.0, 0.5]])
        x2 = jnp.array([[2.0, 0.5]])
        
        out1 = pinn(x1)
        out2 = pinn(x2)
        
        # They shouldn't be exactly equal (very unlikely with random init)
        if jnp.allclose(out1, out2):
    ...
    try:
        ...
        first_layer_weights = pinn.params[0][0]  # Assuming (W, b) structure
        
        assert not jnp.allclose(first_layer_weights, 0.0), \
        ...
        fan_in, fan_out = layers[0], layers[1]
        limit = jnp.sqrt(6.0 / (fan_in + fan_out))
        
        if jnp.all(jnp.abs(first_layer_weights) <= limit * 1.1):  # Allow 10% margin
...
def test_loss_functions(loss_pde, loss_bc, loss_ic, total_loss, Pinn, Geometry, verbose=True):
    ...
    try:
        ...
        points_interior = geo.sample_interior(subkey1, 50)
        points_boundary = geo.sample_boundary(subkey2, 20)
        points_initial = geo.sample_initial(subkey3, 20)
    except Exception as e:
    ...
    # Test 3.1: loss_pde
    try:
        l_pde = loss_pde(pinn.params, pinn, points_interior, D)
        
        assert isinstance(l_pde, (float, jnp.ndarray)), \
            f"loss_pde should return a scalar, got {type(l_pde)}"
        
        if isinstance(l_pde, jnp.ndarray):
            assert l_pde.shape == () or l_pde.shape == (1,), \
                f"loss_pde should return a scalar, got shape {l_pde.shape}"
        
        assert not jnp.isnan(l_pde), "loss_pde returned NaN"
        assert not jnp.isinf(l_pde), "loss_pde returned Inf"
    ...
    try:
        ...
        assert isinstance(l_bc, (float, jnp.ndarray)), \
            f"loss_bc should return a scalar, got {type(l_bc)}"
        
        if isinstance(l_bc, jnp.ndarray):
            assert l_bc.shape == () or l_bc.shape == (1,), \
                f"loss_bc should return a scalar, got shape {l_bc.shape}"
        
        assert not jnp.isnan(l_bc), "loss_bc returned NaN"
        assert not jnp.isinf(l_bc), "loss_bc returned Inf"
    ...
    # Test 3.3: loss_ic
    try:
        l_ic = loss_ic(pinn.params, pinn, points_initial, L)
        
        assert isinstance(l_ic, (float, jnp.ndarray)), \
            f"loss_ic should return a scalar, got {type(l_ic)}"
        
        if isinstance(l_ic, jnp.ndarray):
            assert l_ic.shape == () or l_ic.shape == (1,), \
                f"loss_ic should return a scalar, got shape {l_ic.shape}"
        
        assert not jnp.isnan(l_ic), "loss_ic returned NaN"
        assert not jnp.isinf(l_ic), "loss_ic returned Inf"
    ...
    # Test 3.4: total_loss
    try:
        result = total_loss(pinn.params, pinn, points_interior, points_boundary, 
                           points_initial, D, L)
        ...
        assert not jnp.isnan(l_total), "total_loss returned NaN"
        assert not jnp.isinf(l_total), "total_loss returned Inf"
    ...
    try:
        ...
        computed_total = l_pde_comp + l_bc_comp + l_ic_comp
        
        assert jnp.isclose(l_total, computed_total, rtol=1e-5), \
...
def test_train_function(train_pinn, Pinn, Geometry, verbose=True):
    ...
    try:
        ...
        D = 3.0
        geo = Geometry(L=L, t_start=0.0, t_end=1.0)
        layers = [2, 10, 10, 1]  # Smaller network for faster testing
        key = random.PRNGKey(42)
        pinn = Pinn(key, layers)
        ...
        # Check return values
        assert isinstance(loss_history, (list, np.ndarray, jnp.ndarray)), \
